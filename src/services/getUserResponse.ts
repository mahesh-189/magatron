...
export const getUserResponse = (answerID: string) => {
  ...
  const userResponseBox = createElement("div", {
    className: "user-response-box-container",
  });
  ...
  btn1.setAttribute("btn-name", "like");
  btn2.setAttribute("btn-name", "dislike");
  userResponseBox.append(btn1, btn2);
  chatbotBody.appendChild(userResponseBox);

  btn1.addEventListener("click", (event) => handleResponse(event, answerID));
  btn2.addEventListener("click", (event) => handleResponse(event, answerID));

  async function handleResponse(event: MouseEvent, answerID: string) {
    event.stopImmediatePropagation();
    event.stopPropagation();
    const target = event.currentTarget as HTMLButtonElement;
    const option = target.getAttribute("btn-name");
    option === "like" ? (isSatisfied = true) : (isSatisfied = false);

    const res = await registerUserResponse(answerID, isSatisfied);
    if (res?.data?.success) {
